...
namespace Svg
{
    ...
    public class SvgAttributeAttribute : System.Attribute
    {
        ...
        public string NamespaceAndName
        {
            get
            {
                // ZD: not so nice, but performance is better... 
                switch (_namespace)
                {
                    case SvgNamespace:
                        return _name;
                    case XLinkNamespace:
                        return XLinkPrefix + ":" + _name;
                    case XmlNamespace:
                        return XmlPrefix + ":" + _name;
                    default:
                        throw new InvalidOperationException(String.Format("Unknown namespace: {0}", _namespace));
                }
